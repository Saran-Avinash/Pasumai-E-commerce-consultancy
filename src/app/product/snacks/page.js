...
export default function ProductsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const res = await fetch("https://pasumai-e-commerce-consultancy.vercel.app/api/get-product/snack", {
        cache: "no-store",
      });
  ...
  const addToCart = (product) => {
    const input = prompt(`Enter quantity (pieces) for ${product.name}:`, 1);
  ...
  return (
    <div>
      <div className="text-center my-10">
        <h1 className="text-4xl font-extrabold text-gradient bg-gradient-to-r from-yellow-400 to-red-500 text-transparent bg-clip-text">
          Tasty and Spicy Snacks Await! 🍟🍪🌶️
        </h1>
        <p className="text-gray-500 mt-2">Perfect bites for your cravings!</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 px-4">
        ...
        <div className="w-full lg:w-80 bg-white shadow-xl rounded-xl p-6 h-fit sticky top-6 self-start space-y-6">
          <div>
            <h2 className="text-xl font-bold text-red-700">🛒 Your Cart</h2>
            {cart.length > 0 ? (
              <>
                  ...
                  {cart.map((item, index) => (
                    <li key={index} className="border-b pb-2 text-sm space-y-1">
                      <div className="flex justify-between">
                        <span>
                          {item.name} <span className="text-gray-500">({item.quantity} pcs)</span>
                        </span>
          ...
          <div>
            <h2 className="text-xl font-bold text-red-700">📦 Delivery Details</h2>
